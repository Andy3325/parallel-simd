...
int main()
{
    ...
    bool ok = HipGenerateSegmentValues(
        prefix,
        values,
    ...
    if (!ok)
    {
        cerr << "HIP generate smoke test FAILED: HipGenerateSegmentValues returned false" << endl;
        return 1;
    }
    ...
    for (size_t i = 0; i < expected.size(); ++i)
    {
        ...
        {
            cerr << "HIP generate smoke test FAILED at index " << i
                 << ": expected " << expected[i]
                 << ", got " << guesses[i] << endl;
            return 1;
        }
    ...
    cout << "HIP generate smoke test PASSED" << endl;
    return 0;
}
